"""
Imperfect STM: Realistic LTM + Phonological Loop Capacity Limit

Builds on Realistic LTM. Adds a word-count capacity limit to agent communication.
Direction templates are expanded to 20 sentences across four word-count brackets.
If a sentence exceeds WM_CAPACITY (20) words, random words are dropped before the
brain parses it — potentially corrupting the step counts or direction words.

This is the most biologically complete version, combining the serial position effect
on path memory with a capacity-limited phonological loop for verbal working memory.

Requires a HuggingFace API token.

Grid: 40x40  |  Energy: 100  |  Max steps: 200  |  Food sources: 7  |  Agents: 7
"""
...
# ── HuggingFace Brain Setup ────────────────────────────────
# Add as many tokens as you have — will auto-rotate when one hits quota
HF_TOKENS = [
    "YOUR_TOKEN_HERE",   # token 1
    "YOUR_TOKEN_HERE",   # token 2 (optional)
]
...
# Models from hugging face used for parsing agent directions
HF_MODELS = [
...
DIRECTION_TEMPLATES = [
    ...
    # 16-21 words — pass through untruncated (unless 21 words)
    ...
    # 22-29 words — always will truncate
    ...
    # 30-39 words — always will truncate
...
# In case the LLM runs out of tokens, there is a regex fallback to parse the directions from the agents
...
def parse_directions_hf_brain(agent_response):
    ...
    Falls back to regex if LLM runs out of tokens.
    """
...
class LongTermMemory:
    def __init__(self, parent_memory=None, size=40):
        ...
        # Marks inherited traps in trap_map so the organism avoids them from the start
