"""
Realistic LTM: Serial Position Effect in Path Memory

Builds on Version 4. Models the primacy and recency effects from Burgess and Hitch (1999).
Only the first PRIMACY_STEPS and last RECENCY_STEPS actions of the best path are inherited.
The middle is forgotten. Each organism replays the start, Q-learns through the forgotten
middle, then switches back to the inherited end sequence when close to the goal.

Compare against Perfect LTM to measure the cost of realistic vs perfect episodic memory.
...
PRIMACY_STEPS = 25   # how many steps from the START of the path are remembered
RECENCY_STEPS = 25   # how many steps from the END of the path are remembered
# Steps in between are forgotten (the realistic middle gap)
RECENCY_TRIGGER_DIST = 15  # manhattan distance to goal at which end-replay kicks in
...
def train_organism(ltm, parent_brain=None, organism_offset=0):
    ...
    while True:
        ...
        for step in range(200):
            # Periodically reset epsilon if stuck for too long
            effective_attempt = attempt % 500 if attempt > 500 else attempt
            epsilon = max(0.1, 0.6 - effective_attempt / 300)
            x, y = env.agent_pos
