"""
Perfect LTM: Full Path Sequence Inheritance

Builds on Version 4. On success, the entire action sequence is stored in GLOBAL_BEST_PATH.
Every subsequent organism replays that full path from step one, with a 5% chance of
deviating to a Q-learning action at each step. If a shorter path is found it replaces
the global best. The neural network trains continuously even during replay.

This version represents the theoretical ceiling for sequential memory — perfect recall
of the full route.
