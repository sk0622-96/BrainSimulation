"""
Version 3 Dashboard — Split Graphs
=====================================
...
HISTORY_FILE = "/Users/tommysupey/Desktop/Brain Simulation/version3_history.json"
...
def chart_episodes(runs, all_pg, max_gen, gens):
    ...
    fig.tight_layout()
    save_chart(fig, 'v3_episodes_per_generation.png')


def chart_steps(all_pg, max_gen, gens):
    gen_labels = [f'Gen {g}' for g in gens]
    gen_avg_steps = []
    for g in range(1, max_gen + 1):
        vals = [pg[g-1]['steps'] for pg in all_pg if len(pg) >= g]
        gen_avg_steps.append(sum(vals) / len(vals) if vals else 0)
    ...
    fig.tight_layout()
    save_chart(fig, 'v3_steps_per_generation.png')


def chart_energy(all_pg, max_gen, gens):
    gen_labels = [f'Gen {g}' for g in gens]
    gen_avg_energy = []
    for g in range(1, max_gen + 1):
        vals = [pg[g-1]['energy_remaining'] for pg in all_pg if len(pg) >= g]
        gen_avg_energy.append(sum(vals) / len(vals) if vals else 0)
    ...
    ax.tick_params(axis='x', labelsize=9)
    fig.tight_layout()
    save_chart(fig, 'v3_energy_per_generation.png')


def chart_episodes_bar(all_pg, max_gen, gens):
    gen_labels = [f'Gen {g}' for g in gens]
    gen_avg_episodes = []
    for g in range(1, max_gen + 1):
        vals = [pg[g-1]['episodes'] for pg in all_pg if len(pg) >= g]
        gen_avg_episodes.append(sum(vals) / len(vals) if vals else 0)
    ...
    ax.tick_params(axis='x', labelsize=9)
    fig.tight_layout()
    save_chart(fig, 'v3_episodes_bar.png')
...
def main():
    ...
    chart_episodes_bar(all_pg, max_gen, gens)
    chart_steps(all_pg, max_gen, gens)
    chart_energy(all_pg, max_gen, gens)
...
if __name__ == '__main__':
    print("\n" + "="*50)
    print("  VERSION 3 DASHBOARD — SPLIT GRAPHS")
    print("="*50)
    main()