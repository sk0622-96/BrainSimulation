"""
Version 4: Real Language Communication via HuggingFace

Replaces hardcoded direction queues with a real language model (Qwen2.5-7B-Instruct).
Agents generate natural language sentences; the organism's brain parses them via the HF API.
Organisms are counted cumulatively — the organism number reflects how many total attempts
were made before each success.
...
def save_run_data(organisms, episodes_list):
    history_file = "version4_history.json"
